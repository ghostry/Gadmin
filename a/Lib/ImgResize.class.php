...
class ImgResize {
     ...
     * @return type
     */
    function show_pic($src_img, $dst_w) {
        list($src_w, $src_h, $type) = $this->getimagesize($src_img);  // 获取原图尺寸
        $types = array(1 => "gif", 2 => "jpeg", 3 => "png",);
        $createfrom = "imagecreatefrom" . $types[$type];
        if ($dst_w > $src_w) {
            $dst_w = $src_w;
        }
        $src_scale = $dst_w / $src_w; // 原图和新图比例
        $dst_h = $src_h * $src_scale;
        $source = $createfrom($src_img);
        // 缩放
        $target = imagecreatetruecolor($dst_w, $dst_h);
        $white = imagecolorallocate($target, 255, 255, 255);
        imagefilledrectangle($target, 0, 0, $dst_w, $dst_h, $white);
        imagecolortransparent($target, $white);
        imagecopyresampled($target, $source, 0, 0, 0, 0, $dst_w, $dst_h, $src_w, $src_h);
        return imagejpeg($target);
    }
