...
class Lunar {
    ...
    function toLunar($date = '') {
        if ($date) {
            $arr = $this->convertSolarToLunar(date('Y', strtotime($date)), date('m', strtotime($date)), date('d', strtotime($date)));
        } else {
            $arr = $this->convertSolarToLunar(date('Y'), date('m'), date('d'));
        }
        return $arr[3] . $arr[6] . '年' . $arr[1] . $arr[2];
    }
